...
<script>
	var routeDatatable = "{{route('elearning.tahunAjaran.main')}}";
	var routeTahunAjaranAdd = "{{route('elearning.tahunAjaran.add')}}";
	var routeTahunAjaranDelete = "{{route('elearning.tahunAjaran.add')}}";
	...
	async function dataTable() {
		...
		const templateTambah = `
			<button onclick="tambahTahunAjaran()" class='btn btn-primary p-2 w-100'><i class='bx bx-plus' ></i>Tambah</button>
		`;

		$("div.templateTambah").html(templateTambah)
	}

	function tambahTahunAjaran(id='') {
		$('.main-page').hide();
		var url = routeTahunAjaranAdd
		$.post(url, {id:id})
		.done(function(data){
	...
	function hapusTahunAjaran(id) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				var url = routeTahunAjaranDelete
				$.post(url, {id:id})
				.done(function(data){
					console.log(data);
					if(data.code == 200){
