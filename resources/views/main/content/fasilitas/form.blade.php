		...
		<div class="card">
			...
			<div class="card-body">
				<form id="formFasilitas">
					<input type="hidden" name="id" @isset($fasilitas) value="{{$fasilitas->id_exkul}}" @endisset>
					<div class="row">
						<div class="col-8">
							<div class="mb-3">
								<label for="nama_exkul" class="form-label">Nama Fasilitas *</label>
								<input type="text" class="form-control" name="nama_exkul" id="nama_exkul" placeholder="Nama Fasilitas" @isset($fasilitas) value="{{$fasilitas->nama_exkul}}" @endisset>
							</div>
						</div>
						<div class="col-4">
							<div class="mb-3">
								...
								<select name="status_exkul" id="status_exkul" class="form-control selectpicker select2">
									...
									<option value="1" @isset($fasilitas) @if($fasilitas->status_exkul) selected @endif @endisset>AKTIF</option>
									<option value="0" @isset($fasilitas) @if(!$fasilitas->status_exkul) selected @endif @endisset>TIDAK AKTIF</option>
								</select>
							</div>
						</div>
					</div>
					<div class="row mb-3">
						<div class="col-md-12">
							{{-- <label for="isi">fasilitas</label> --}}
							<textarea id="berita" name="berita" rows="40" cols="100" class="form-control">@isset($fasilitas) {{$fasilitas->deskripsi}} @endisset</textarea>
						</div>
					</div>
					<div class="mb-3">
						<label for="foto" class="form-label">Gambar *</label>
						<center class="mb-3">
							<a class="pan" id="btnOutPut" data-big="@isset($fasilitas){!! url('uploads/exkul/'.$fasilitas->foto) !!}@endisset">
								<img id="outPut" @isset($fasilitas) src="{!! url('uploads/exkul/'.$fasilitas->foto) !!}" @endisset class="rounded mx-auto d-block responsive @isset($fasilitas) img-thumbnail w-50 @endisset">
							</a>
