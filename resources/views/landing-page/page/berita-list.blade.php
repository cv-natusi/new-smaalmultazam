	...
	<ul class="list-unstyled items berita-area">
		...
		<li class="item">
			<div class="row gutter-width-xs">
				<div class="col-3">
					<a href="{{ route(Str::camel($curNav).'.'.Str::camel($curMenu)) }}/{{$item->id_berita}}">
						<div class="img object-fit overflow-hidden">
							<div class="object-fit-cover transform-scale-h">
								<img src="{{ asset($pathGambar.$item->gambar)}}" alt="5 Tips for Sustainable Business Success">
							</div>
						</div>
					</a>
				</div>

				<div class="col-9 align-self-center font-rubik">
					<h6 class="item-t-head font-weight-bold main-text-green"><a title="" href="{{ route(Str::camel($curNav).'.'.Str::camel($curMenu)) }}/{{$item->id_berita}}">{{$item->judul}}</a></h6>
					<a style="font-size: 10pt" href="#" class="text-secondary">{{isset($item->tanggal)?date('d F Y',strtotime($item->tanggal)):'-'}}</a>
...
<script>
	...
	$(document).ready( function () {
		page = parseInt(JSON.parse('{{$berita->currentPage()}}'))
		size = parseInt(JSON.parse('{{$berita->lastPage()}}'))
		prevUrl = '{{$berita->previousPageUrl()}}'
		nextUrl = '{{$berita->nextPageUrl()}}'
		pageUrl = '{{$berita->url(1)}}'
