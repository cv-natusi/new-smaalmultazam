...
namespace App\Http\Controllers\Main\MenuUtama\AgendaEvent;
...
class AgendaEventController extends Controller
{
		...
		if ($request->ajax()) {
			return DataTables::of($data)->
				...
				addColumn('actions',function($row){
					$html = "<button onclick='tambahAgendaEvent($row->id_berita)' class='btn btn-dark btn-purple p-2'><i class='bx bx-edit-alt mx-1'></i></button>";
					if ($row->status) {
						$html .= "<button onclick='aktifAgendaEvent($row->id_berita)' class='btn ms-1 btn-secondary p-2'><i class='bx bx-power-off mx-1'></i></button>";
					} else {
						$html .= "<button onclick='aktifAgendaEvent($row->id_berita)' class='btn ms-1 btn-primary p-2'><i class='bx bx-power-off mx-1'></i></button>";
					}
					$html .= "<button onclick='hapusAgendaEvent($row->id_berita)' class='btn ms-1 btn-danger p-2'><i class='bx bx-trash mx-1'></i></button>";
					return $html;
				})->
				rawColumns(['actions'])->toJson();
		}
		$data['curNav'] = 'Menu Utama';
		$data['curMenu'] = 'Agenda Event';
		$data['title'] = 'Agenda Event';
		return view('main.content.agenda.main',$data);
	}

	public function add(Request $request) {
		$data['berita'] = Berita::find($request->id);
		$content = view('main.content.agenda.form',$data)->render();
	...
	public function save(Request $request) {
		...
		$message = [
			...
			'tanggal_acara.required'=>'Kolom Tanggal Wajib Diisi',
