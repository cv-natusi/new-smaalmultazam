...
namespace App\Http\Controllers\Main\ProgramSekolah\Ekstrakulikuler;
...
class EkstrakulikulerController extends Controller
{
	...
	public function __construct()
	{
		...
		$this->data['curMenu'] = 'Ekstrakurikuler';
		$this->data['title'] = 'Ekstrakurikuler';
	}

	public function main(Request $request) {
		$status = isset($request->status) ? $request->status : '';
		$exkul = Exkul::where('type_exkul',1)
			...
			->get();
		if ($request->ajax()) {
			return DataTables::of($exkul)->
				addIndexColumn()->
				addColumn('status',function($row){
					return $row->status_exkul ? 'Aktif' : 'Tidak Aktif';
				})->
				addColumn('actions',function($row){
					$html = "<button onclick='tambahEkstrakulikuler($row->id_exkul)' class='btn btn-dark btn-purple p-2'><i class='bx bx-edit-alt mx-1'></i></button>";
					if ($row->status_exkul) {
						$html .= "<button onclick='aktifEkstrakulikuler($row->id_exkul)' class='btn ms-1 btn-secondary p-2'><i class='bx bx-power-off mx-1'></i></button>";
					} else {
						$html .= "<button onclick='aktifEkstrakulikuler($row->id_exkul)' class='btn ms-1 btn-primary p-2'><i class='bx bx-power-off mx-1'></i></button>";
					}
					$html .= "<button onclick='hapusEkstrakulikuler($row->id_exkul)' class='btn ms-1 btn-danger p-2'><i class='bx bx-trash mx-1'></i></button>";
					return $html;
				})->
				rawColumns(['actions'])->toJson();
			}
		$data = $this->data;
		return view('main.content.ekstrakulikuler.main',$data);
	}

	public function add(Request $request) {
		$data = $this->data;
		$data['exkul'] = Exkul::find($request->id);
		$content = view('main.content.ekstrakulikuler.form',$data)->render();
