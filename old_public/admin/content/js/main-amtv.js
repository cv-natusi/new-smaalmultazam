...
async function dataTable(status='') {
    ...
    await $('#dataTable').DataTable({
		...
		columns: [
			...
			{data:'judul_amtv', name:'judul_amtv'},
			{data:'file', name:'file'},
			{data:'status_amtv', name:'status_amtv'},
    ...
    const templateStatus = `
		<div class="d-flex">
			...
			<select name="status" aria-controls="status" class="form-select form-select-sm" id="status">
...
function tambahAmtv(id='') {
	$('.main-page').hide();
	var url = routeAmtvAdd
	$.post(url, {id:id})
	.done(function(data){
...
function aktifAmtv(id) {
	var url = routeAmtvAktif
	$.post(url, {id:id})
	.done(function(data){
		console.log(data);
		if(data.code == 200){
