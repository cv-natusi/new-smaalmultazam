...
async function dataTable(status='') {
    ...
    await $('#dataTable').DataTable({
		...
		columns: [
			...
			{data:'judul_reels', name:'judul_reels'},
			{data:'file', name:'file'},
			{data:'status_reels', name:'status_reels'},
...
function aktifReels(id) {
	var url = routeReelsAktif
	$.post(url, {id:id})
	.done(function(data){
		console.log(data);
		if(data.code == 200){
